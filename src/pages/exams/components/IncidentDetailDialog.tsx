      ...
      {incident && (
        <Dialog.Panel className="max-w-md">
          ...
          <Dialog.Content className="max-h-[62vh] overflow-y-auto">
            ...
            <div className="mt-4">
              <SectionLabel className="mb-2">{incident.subjects.length > 1 ? t('detail.subjectsPairLabel') : t('detail.subjectLabel')}</SectionLabel>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="bg-muted-100 h-1.5 flex-1 overflow-hidden rounded-full">
                    <div
                      ...
                      style={{ width: `${Math.round(incident.conf * 100)}%` }}
                    />
                  </div>
                </div>
              </div>
          ...
          <Dialog.Footer className="flex flex-col gap-2">
            {incident.status === 'open' && (
              <div className="flex w-full gap-2">
                <Button variant="success" className="flex-1" onClick={() => onConfirm(incident.id)}>
                  <Check size={15} />
                  {t('detail.confirm')}
                </Button>

                <Button variant="outline-muted" className="flex-1" onClick={() => onDiscard(incident.id)}>
                  {t('detail.discard')}
                </Button>
              </div>
            )}
            ...
            {incident.status === 'confirmed' && (
              <Button variant="outline-muted" onClick={() => onDiscard(incident.id)}>
                {t('detail.discardInstead')}
              </Button>
            )}

            {incident.status === 'discarded' && (
              <Button variant="outline-muted" onClick={() => onRestore(incident.id)}>
                <RotateCcw size={14} />
                {t('detail.restore')}
              </Button>
            )}

            <Button variant="outline" onClick={() => onProof(incident.id)}>
              <FileText size={15} />
